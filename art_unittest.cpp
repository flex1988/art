...
TEST(art, LeafNode_Leaf16_Insert)
{
    AdaptiveRadixTree* art = new AdaptiveRadixTree;
    art->Init();
...
{
    AdaptiveRadixTree* art = new AdaptiveRadixTree;
    art->Init([](void* v) -> void { assert(v == (void*)1); }, [](void* v) -> void {});
    art->RangeInsert(0, 100, (void*)1);
    art->RangeInsert(0, 100, (void*)2);
    art->Destroy();
    delete art;
}
...
{
    AdaptiveRadixTree* art = new AdaptiveRadixTree;
    art->Init([](void* v) -> void {}, [](void* v) -> void { (*(int*)v)++; });
    art->RangeInsert(0, 256, (void*)0);
    for (int i = 0; i < 256; i++)
    {
        checkValueRange(art, i, 1, (void*)i);
    }
    art->Destroy();
    delete art;
}
